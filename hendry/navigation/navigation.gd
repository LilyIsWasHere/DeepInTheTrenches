...
func sample_steering(agent: Node, handle: NavPlanHandle, use_xz_only: bool = false) -> NavSteeringResult:
	...
	var agent_node: Node3D = agent
	var current_position: Vector3 = agent_node.global_position * Vector3(1,1,1)#(Vector3(1,0,1) if sample_2d else Vector3(1,1,1))
	...
	var next_waypoint: Vector3 = handle.waypoints[0] * Vector3(1,1,1)#(Vector3(1,0,1) if sample_2d else Vector3(1,1,1))
