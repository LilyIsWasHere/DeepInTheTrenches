...
var shader_file_regex: RegEx = RegEx.new()
...
var shader_compilations: Dictionary[String, RID] = {}
var shader_code_cache: Dictionary[String, String] = {}

var compute_shader_kernel_compilations: Dictionary[String, Array] = {}
...
func compile_shader(shader_file_path: String) -> void:
...
func compile_compute_shader(compute_shader_file_path: String) -> void:
	...
	var raw_shader_code: PackedStringArray= raw_shader_code_string.split("\n")
	
	var kernel_names: Array = Array()
	...
	for i in raw_shader_code.size():
		...
		for kernel_name: String in kernel_names:
			if line.contains(kernel_name) and line.contains('void'):
				...
				var newLine: String = raw_shader_code[i - 1].strip_edges()
				if newLine.contains('numthreads'):
					var thread_groups: PackedStringArray = newLine.split('(')[-1].split(')')[0].split(',')
	...
	for kernel_name: String in kernel_names:
		var shader_code: PackedStringArray = PackedStringArray(raw_shader_code)
...
func _init() -> void:
	...
	for shader_file: String in shader_files:
		compile_shader(shader_file)

	for file_path: String in compute_shader_file_paths:
...
func _process(delta: float) -> void:
	...
	for file_path: String in compute_shader_file_paths:
		if shader_code_cache[file_path] != FileAccess.open(file_path, FileAccess.READ).get_as_text():
			var shader_name: String = get_shader_name(file_path)

			# Free existing kernels
			for kernel: RID in compute_shader_kernel_compilations[shader_name]:
				rd.free_rid(kernel)
...
func _notification(what: int) -> void:
	if what == NOTIFICATION_PREDELETE or what == NOTIFICATION_WM_CLOSE_REQUEST:
		...
		for shader_name: String in shader_names:
			var shader: RID = shader_compilations[shader_name]
			if shader.is_valid():
				print("Freeing: " + shader_name)
				rd.free_rid(shader)

		for compute_shader: String in compute_shader_kernel_compilations.keys():
			for kernel: RID in compute_shader_kernel_compilations[compute_shader]:
				rd.free_rid(kernel)
...
func get_compute_kernel_compilation(shader_name: String, kernel_index: int) -> RID:
	return compute_shader_kernel_compilations[shader_name][kernel_index]

func get_compute_kernel_compilations(shader_name: String) -> Array:
