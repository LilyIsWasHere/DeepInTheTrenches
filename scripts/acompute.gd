...
# Binding -> Array
var uniform_buffer_cache: Dictionary = {}
# Contains the RIDs for the gpu versions of the uniform array
# Binding -> RID
var uniform_buffer_id_cache: Dictionary = {}
...
func _init(_shader_name: String) -> void:
	...
	for kernel: RID in AcerolaShaderCompiler.get_compute_kernel_compilations(shader_name):
...
func free() -> void:
	for kernel: RID in kernels:
		rd.free_rid(kernel)

	for binding: int in uniform_buffer_id_cache.keys():
